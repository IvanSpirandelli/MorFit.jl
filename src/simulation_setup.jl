...
function _get_realization_radii_and_sizes(x, input)
    # Unified format: template_centers and template_radii are both lists (one per molecule)
    realz = Utilities.get_flat_realization(x, input["template_centers"])
    sizes = [length(r) for r in input["template_radii"]]
    radii = vcat(input["template_radii"]...)
    return realz, sizes, radii
end
...
function get_energy(input)
    ...
    elseif input["energy"] == "fsol"
        # Pass template_radii as list - _prepare_args handles flattening
        return (x) -> Energies.solvation_free_energy_and_measures_in_bounds(x, input["template_centers"], input["template_radii"], input["rs"], input["prefactors"], input["overlap_jump"], input["overlap_slope"], input["bounds"], input["delaunay_eps"])
...
function get_connected_component_solvation_free_energy_in_bounds_energy_call(input)
    ...
    if input["n_mol"] == 2
        bol_nmol = (x) -> Energies.are_bounding_spheres_overlapping(x, 1, 2, Energies.get_bounding_radii(template_centers, template_radii, rs))
        return (x) -> Energies.solvation_free_energy_and_measures_with_bounding_container_check_in_bounds(
            x, template_centers, template_radii,
