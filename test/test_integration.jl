...
# Shared setup: params, precomputed values, and energy closure for a 2-molecule system
function _energy_setup(centers, radii)
    ...
    return (; system, sol, ol, topo, num, scales, prefactors, bounding_radii,
            single_energies, single_measures, precomputed, bol_check, energy_fn)
end

_well_separated_state() = [
    (one(QuatRotation), [50.0, 50.0, 50.0]),
    (one(QuatRotation), [150.0, 150.0, 150.0])
]
...
function run_integration_tests()
    ...
    @testset "Precomputation" begin
        setup = _energy_setup(centers, radii)
        @test length(setup.prefactors) == 4
        @test length(setup.bounding_radii) == 2
        @test length(setup.single_energies) == 2
        @test length(setup.single_measures) == 2
    end

    @testset "Energy Calculation" begin
        setup = _energy_setup(centers, radii)

        energy, measures = setup.energy_fn(_well_separated_state())
    ...
    @testset "RWM Simulation" begin
        ...
        x_init = _well_separated_state()
        output = Algorithms.SimulationOutput()

        β = 1.0 / 1.3  # temperature = 1.3
        rwm = Algorithms.RandomWalkMetropolis(setup.energy_fn, perturbation_fn, β)
        ...
        # SimulationOutput reconstruction from the saved Dict
